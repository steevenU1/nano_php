...
function validarTraslape($conn, $id, $marca, $modelo, $capacidad, $f1, $f2) {
  // Regresa true si hay traslape activo con otro registro
  $sql = "SELECT COUNT(*) c
          FROM comisiones_especiales
          WHERE activo=1
            AND marca=? AND modelo=? AND IFNULL(capacidad,'')=IFNULL(?, '')
            AND (
                 (fecha_inicio <= ? AND fecha_fin >= ?) OR   -- f1 dentro
                 (fecha_inicio <= ? AND fecha_fin >= ?) OR   -- f2 dentro
                 (fecha_inicio >= ? AND fecha_fin <= ?)      -- abarca por completo
            )";
  if ($id) $sql .= " AND id<>?";
  $stmt = $conn->prepare($sql);
  if ($id) {
    $stmt->bind_param("ssssssssi", $marca,$modelo,$capacidad,$f1,$f1,$f2,$f2,$f1,$f2,$id);
  } else {
    $stmt->bind_param("ssssssss", $marca,$modelo,$capacidad,$f1,$f1,$f2,$f2,$f1,$f2);
  }
  $stmt->execute();
  ...
  if (!hash_equals($_SESSION['csrf'], $_POST['csrf'] ?? '')) {
    ...
    try {
      if ($accion === 'crear') {
        ...
        $f1 = $_POST['fecha_inicio']; $f2 = $_POST['fecha_fin'];
          ...
          <div class="table-responsive">
            <table class="table table-hover align-middle">
              <thead class="table-light">
                <tr>
                  <th>#</th><th>Equipo</th><th>Vigencia</th><th>Monto</th><th>Activo</th><th class="text-end">Acciones</th>
                </tr>
              </thead>
              <tbody>
                <?php if (!$rows): ?>
