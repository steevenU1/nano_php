...
<head>
  ...
  <style>
    ...
    #tablaDetalle .col-codigo {
      min-width: 330px;
    }

    #tablaDetalle .col-color {
      width: 130px;
    }

    #tablaDetalle .col-ram {
      width: 110px;
    }

    #tablaDetalle .col-cap {
      width: 140px;
    }

    #tablaDetalle .col-qty {
      width: 90px;
    }
    ...
    #tablaDetalle .col-ivp {
      width: 110px;
    }

    #tablaDetalle .col-sub {
      width: 130px;
    }

    #tablaDetalle .col-iva {
      width: 120px;
    }

    #tablaDetalle .col-tot {
      width: 140px;
    }

    #tablaDetalle .col-req {
      width: 140px;
      text-align: center;
    }

    #tablaDetalle .col-dto {
      width: 110px;
      text-align: center;
    }

    #tablaDetalle .col-acc {
      width: 64px;
    }

    #tablaDetalle .form-control {
      padding: .35rem .55rem;
    }

    #tablaDetalle input.num {
      text-align: right;
    }
    ...
    #tablaCargos .form-control {
      padding: .35rem .55rem;
    }

    #tablaCargos input.num {
      text-align: right;
    }
  ...
  <script>
    ...
    `;
      ...
      const ivpReactive = () => {
        ...
        if (modalShown && currentDtoRow === tr) {
          syncDtoIva();
        }
      };
    ...
    btnGuardarDto.addEventListener('click', () => {
      ...
      if (!isFinite(v1) || v1 <= 0) {
        alert('Costo Descuento inválido');
        return;
      }
      if (!isFinite(v2) || v2 <= 0) {
        alert('Costo Descuento c/IVA inválido');
        return;
      }
    ...
    document.getElementById('formCompra').addEventListener('submit', function(e) {
      ...
      rows.forEach(tr => {
        ...
        if (!hidden.value) {
          ok = false;
          msg = 'Verifica que todos los renglones tengan un código válido.';
        }
        if (!isFinite(qty) || qty < 1) {
          ok = false;
          msg = 'Hay cantidades inválidas (deben ser ≥ 1).';
        }
        if (!isFinite(pu) || pu <= 0) {
          ok = false;
          msg = 'Hay precios unitarios en 0 o inválidos.';
        ...
        if (chk && chk.checked) {
          ...
          if (!v1 || !v2) {
            ok = false;
            msg = 'Hay renglones con “Desc.” activo sin capturar costos de descuento.';
          }
        }
      });
