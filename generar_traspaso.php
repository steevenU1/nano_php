...
// detalle_traspaso         (ya existente) → id, id_traspaso, id_inventario
// detalle_traspaso_acc     (nuevo)        → id, id_traspaso, id_inventario_origen, id_producto, cantidad
$conn->query("
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if ($idSucursalDestino<=0) {
    ...
    try {
      ...
      if (!empty($accQty)) {
        // Traer renglones de inventario origen con cantidad y producto (sin IMEIs)
        $place = implode(',', array_fill(0, count(array_keys($accQty)), '?'));
        $types = str_repeat('i', count($accQty));
        $sqlAcc = "
          SELECT i.id, i.id_producto, i.cantidad, p.marca, p.modelo, p.imei1
          FROM inventario i
...
FROM inventario i
JOIN productos p ON p.id = i.id_producto
WHERE i.id_sucursal=? AND i.estatus='Disponible' AND (p.imei1 IS NOT NULL AND p.imei1 <> '')
...
$sqlAccList = "
SELECT i.id, i.id_producto, i.cantidad, p.marca, p.modelo, p.color
FROM inventario i
JOIN productos p ON p.id = i.id_producto
...
<head>
  ...
  <style>
    ...
    /* Overlay de carga para el acuse */
          ...
          <div class="tab-pane fade" id="tab-accesorios" role="tabpanel">
            <div class="card mt-2">
              ...
              <div class="card-body pt-2">
                <div class="input-group mb-2">
                  <span class="input-group-text"><i class="bi bi-search"></i></span>
                  <input type="text" id="buscadorAcc" class="form-control" placeholder="Buscar por marca o modelo...">
                ...
                <div class="table-responsive" style="max-height:420px; overflow:auto;">
                  <table class="table table-hover align-middle mb-0" id="tablaAcc">
                    <thead class="table-light position-sticky top-0">
                      <tr>
                        <th>ID Inv</th><th>Marca</th><th>Modelo</th><th>Color</th><th class="text-end">Disp.</th><th class="text-end">Traspasar</th>
                      </tr>
                    </thead>
                    <tbody>
                    <?php if(empty($accesorios)): ?>
                      <tr><td colspan="6" class="text-center text-muted py-4">Sin accesorios disponibles</td></tr>
                    <?php else: foreach($accesorios as $row): ?>
                      <tr data-id="<?= (int)$row['id'] ?>" data-marca="<?= h($row['marca']) ?>" data-modelo="<?= h($row['modelo']) ?>" data-color="<?= h($row['color']) ?>" data-disp="<?= (int)$row['cantidad'] ?>">
                        <td class="fw-semibold"><?= (int)$row['id'] ?></td>
                        <td><?= h($row['marca']) ?></td>
                        <td><?= h($row['modelo']) ?></td>
                        <td><span class="chip"><?= h($row['color']) ?></span></td>
...
document.getElementById('clearEq').addEventListener('click', ()=>{document.getElementById('buscadorEq').value=''; filtra('buscadorEq','tablaEquipos');});
document.getElementById('buscadorAcc').addEventListener('keyup', ()=>filtra('buscadorAcc','tablaAcc'));
document.getElementById('clearAcc').addEventListener('click', ()=>{document.getElementById('buscadorAcc').value=''; filtra('buscadorAcc','tablaAcc');});
...
document.querySelectorAll('#tablaAcc .btn-acc-inc').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    if (v < max) { input.value = v+1; input.dispatchEvent(new Event('input')); }
  });
});
document.querySelectorAll('#tablaAcc .btn-acc-dec').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    const input = btn.parentElement.querySelector('input.qty-input');
    let v = parseInt(input.value,10) || 0;
    if (v > 0) { input.value = v-1; input.dispatchEvent(new Event('input')); }
  });
});
...
// disparadores para equipos
document.querySelectorAll('#tablaEquipos .chk-eq').forEach(chk=>{
  chk.addEventListener('change', rebuildCart);
});

// Vaciar carrito (desmarca y pone 0)
...
(function(){
  ...
  // Mostrar modal y comenzar carga
  setBusy(true);
  frame.src = 'about:blank'; // reset seguro
  modal.show();

  // Al cargar el iframe, ocultar overlay
  let done = false;
  ...
  // Fallback: si algo va mal con el evento 'load', libera en 7s
  const fallback = setTimeout(()=>{ if (!done) setBusy(false); }, 7000);

  // Inicia la navegación al acuse (después de enganchar eventos)
