<?php
// navbar.php — versión “bonita” con activo automático y nombre corto
...
function resolveAvatarUrl(?string $fotoBD): ?string {
  ...
  $baseUri = '';
  if ($docroot && str_starts_with($appDir, $docroot)) { $baseUri = substr($appDir, strlen($docroot)); }
  ...
  // Ruta web absoluta
  if (str_starts_with($f,'/')) { if ($docroot && is_file($docroot.$f)) return $f; return $f; }
  // Relativas
...
if ($idUsuario > 0) {
  ...
  if ($st->fetch()) { $avatarUrl = resolveAvatarUrl($fotoBD); }
  $st->close();
}
...
$grpVentasLog  = ['historial_ventas.php','historial_ventas_sims.php'];
$grpVentas     = array_merge($grpVentasLog, ['nueva_venta.php','venta_sim_prepago.php','venta_sim_pospago.php']);
...
<style>
  ...
  .navbar.pretty .navbar-brand{
    font-weight:700; letter-spacing:.2px; color: var(--nb-text);
  }
  .navbar.pretty .navbar-brand .brand-title{
    background: linear-gradient(90deg,#a8e6ff,#7cf6d7);
    -webkit-background-clip:text;background-clip:text;
    -webkit-text-fill-color:transparent;
  }
  .navbar.pretty .nav-link{
    color: var(--nb-text); opacity:.92; position:relative;
    transition: opacity .2s ease, transform .2s ease;
  }
  .navbar.pretty .nav-link:hover{ opacity:1; transform: translateY(-1px); }
  ...
  .navbar.pretty .nav-link:hover::after{ transform: scaleX(1); }
  .navbar.pretty .nav-link.active,
  .navbar.pretty .dropdown-item.active{
    color:#fff !important; font-weight:600;
  }
  .navbar.pretty .nav-link.active::after{ transform: scaleX(1); }
  ...
  .dropdown-menu-dark .dropdown-item{
    border-radius: 10px; color: var(--nb-text);
  }
  .dropdown-menu-dark .dropdown-item:hover{
    background: var(--nb-hover); color: #fff;
  }
  .dropdown-menu-dark .dropdown-header{
    color: var(--nb-muted); font-size:.78rem; letter-spacing:.4px;
  ...
  /* --- Brand title: gradiente animado sutil --- */
.brand-title{
...
<nav class="navbar navbar-expand-lg navbar-dark sticky-top shadow pretty">
  <div class="container-fluid">
    ...
    <a class="navbar-brand d-flex align-items-center" href="dashboard_unificado.php">
      <img src="https://i.ibb.co/kgT18Gx8/4e68073d-5ca3-4c71-8bf5-9756511d74fb.png" alt="Logo" width="36" height="36" class="me-2" style="object-fit:contain;">
      ...
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <li class="nav-item dropdown">
          ...
          <ul class="dropdown-menu dropdown-menu-dark">
            ...
            <li><a class="dropdown-item<?= isActive(['cortes_caja.php']) ?>" href="cortes_caja.php"><i class="bi bi-scissors me-2"></i>Corte de caja</a></li>
      ...
      <ul class="navbar-nav ms-auto">
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" role="button" data-bs-toggle="dropdown">
            ...
            <!-- 👇 Se quitó la sucursal en la topbar para ahorrar espacio -->
          </a>
